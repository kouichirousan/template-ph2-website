  ...
  <main class="l-main">
    ...
    <div class="p-quiz-container l-container">
      <?php for ($i = 0; $i < count($questions); $i++) { ?>
        <section class="p-quiz-box js-quiz" data-quiz="<?php echo $i; ?>">
          <div class="p-quiz-box__question">
            <h2 class="p-quiz-box__question__title">
              <span class="p-quiz-box__label">Q<?php echo $i + 1; ?></span>
              <span class="p-quiz-box__question__title__text"><?php echo htmlspecialchars($questions[$i]['content'], ENT_QUOTES, 'UTF-8'); ?></span>
            </h2>
            <?php if (!empty($questions[$i]['image'])) { ?>
              <figure class="p-quiz-box__question__image">
                <img src="../assets/img/quiz/<?php echo htmlspecialchars($questions[$i]['image'], ENT_QUOTES, 'UTF-8'); ?>" alt="">
              </figure>
            <?php } ?>
          </div>
            ...
            <ul class="p-quiz-box__answer__list">
              <?php foreach ($questions[$i]['choices'] as $choiceIndex => $choice) { ?>
                <li class="p-quiz-box__answer__item">
                  <button class="p-quiz-box__answer__button js-answer" data-answer="<?php echo htmlspecialchars($choice['valid'], ENT_QUOTES, 'UTF-8'); ?>">
                    <?php echo htmlspecialchars($choice['name'], ENT_QUOTES, 'UTF-8'); ?><i class="u-icon__arrow"></i>
                  </button>
                </li>
              <?php } ?>
            </ul>
          ...
          <?php if (!empty($questions[$i]['supplement'])) { ?>
            <cite class="p-quiz-box__note">
              <i class="u-icon__note"></i><?php echo htmlspecialchars($questions[$i]['supplement'], ENT_QUOTES, 'UTF-8'); ?>
            </cite>
          <?php } ?>
        </section>
      <?php } ?>
